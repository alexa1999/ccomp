...
pointArray::pointArray(const point<TP> ptr[],const int size){
    this->size=size;
    this->points=new point[size];
...
void pointArray::push_back(const point<TP> &p)
{
    resize(size +1);
    points[size-1]=p;
}
void pointArray::insert (const int pos,const point<TP> &p)
{
